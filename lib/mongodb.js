...
MongoDB.prototype.aggregate = function aggregate(model, groupBy, callback) {
	this.collection(model).aggregate([groupBy],
		function(err, data){
			if (err) return callback(err);
			callback(err, data.map(function (o) { o.id = o._id; return o; }));
		}
	);
};
